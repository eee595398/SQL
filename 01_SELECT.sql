...
-- EMPLOYEE 테이블에서 모든 사원의 사번, 이름, 급여, 연봉조회
...
-- DAUL(DUmy tAbLe) 테이블 : 가짜 테이블(임시 조회용 테이블)
...
SELECT SYSDATE  -1 "하루 전", SYSDATE AS 현재시잔, SYSDATE +1 내일
FROM DUAL;
...
-- DISTINCT : 조회시 컬럼에 포함된 중복 값을 한 번만 표기
...
--3번 SELECT 절 : SELECT 컬럼명
--해석 순서 1번 FROM 절 : FROM 테이블명
--2번 WHERE 절(조건절) : WHERE 컬러명 연산자 값
-- 4번 ORDER BY 컬럼명 : 별칭 : 컬럼 순서[ASC | DESC] [NULLS FIRST | LAST]
...
FROM EMPLOYEE
WHERE SALARY < 30000000 OR SALARY >=50000000;
...
--EMPLOYEE 테이블에서 EMAIL의 _앞에 글자 세글자인 사원만 조회
SELECT EMAIL,EMP_NAME 
FROM EMPLOYEE
WHERE EMAIL LIKE '____%';
-- ESCAPE 
-- ESAPE 문자 뒤에 작성된 _는 일반문자로 탈출한다는 뜻
-- #,^
SELECT EMAIL,EMP_NAME 
FROM EMPLOYEE
...
(DEPT_CODE ='D9' OR DEPT_CODE = 'D6') AND HIRE_DATE BETWEEN '1990-01-01' AND '2000-12-31'
AND SALARY <=27000000;
 ...
 */
 ...
 SELECT EMP_NAME, BONUS 
 FROM EMPLOYEE
 WHERE BONUS IS NULL; -- 14행 밑에 써있음

--------------------
 
 /* ORDER BY 절
  * 
  * - SELECT문의 조회 결과(RESULT SET)를 정렬할때 사용하는 구문
  * 
 ...
 FROM EMPLOYEE
 ORDER BY SALARY; -- ASC가 기본값 [ASC | DESC] // ASC 오름차순 
