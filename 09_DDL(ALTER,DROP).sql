...
-- * 수정은 별도 존재하지 않는다 --> 삭제 후 추가르 이용해서 수정

--- DEPARTMENT 테이블 복사(컬럼명, 테이터 타입, NOT NULL만 )
CREATE TABLE DEPT_COPY
AS SELECT * FROM DEPARTMENT;

SELECT * FROM DEPT_COPY;

--DEPT_COPY의 DEPT_TITLE 칼럼에 UNIQUE 추가 

ALTER TABLE DEPT_COPY 
ADD CONSTRAINT DEPT_COY_TITLE_U UNIQUE(DEPT_TITLE);




-- DEPT_COPY의 DEPT_TITLE 칼럼에 UNIQUE 삭제

ALTER TABLE DEPT_COPY
DROP CONSTRAINT DEPT_COY_TITLE_U;


-- *** DEPT_COPY의 DEPT_TITLE 컬럼에 NOT NULL 제약조건 추가/삭제 

ALTER TABLE DEPT_COPY
ADD CONSTRAINT DEPT_COPY_U NOT NULL(DEPT_TITLE);
--> NOT NULL 제약조건은 새로운 제약조건을 추가하는 것이 아니다 
-- 컬럼 자체에 NULL 허용/비허용을 제어하는 성질 변경의 형태로 인식한다 

-- MODIFY(수정하다) 구문을 사용해서 NULL 제어 
ALTER TABLE DEPT_COPY
MODIFY DEPT_TITLE NOT NULL; -- DEPT_TITLE 칼럼을 NOT NULL로 수정 
...
--- 2. 컬럼(추가/수정/삭제)
--컬럼 추가
--ALTER TABLE 테이블명 AAD(컬럼명 데이터 타입 [DEFAULT '값']);
...
-- ALTER TABLE 테이블명 MODIFY 컬러명 NULL / NOT NULL; NULL 여부 변경 
...
--> 기본 값을 변경 했다고 해서 기존 데이터가 변하지는 않는다 
...
-- DEPT_COPY 테이블에 PK추가 (컬럼 : DEPT_ID 제약 조건명 : D_COPY_PK)
ALTER TABLE DEPT_COPY
...
-- 3) DROP TABLE 삭제옵션 CASCADE CONSTRAINTS 사용 
		--> 삭제하려는 테이블과 연결된 FK 제약 조건을 모두 삭제 
...
-- DDL 수행 중 에 DHL 수행을 하면 DML이 COMMIT을 수행하지 않아도 자동 COMMIT 
