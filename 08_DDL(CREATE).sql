...
-- 데이터의 전체 구조를 정의하는 언어로 주로 DB관리자, 설계자가 사용한다


-- 오라클에서의 객체 : 테이블(TABLE), 뷰(VIEW), 시퀀스(SEQUENCE), 인덱스(INDEX), 패키지(PACKAGE)
-- 트리거(TRIGGER), 프로시져(PROCEDURE), 함수(FUNCTION), 동의어(SYNONYM), 사용자
...
-- 테이블이란
-- 행(ROW)과열(COLUMN)으로 구성되는 가장 기본적인 데이터베이스 객체
-- 데이터 베이스 내에서 모든 데이터는 테이블을 통해서 저장된다

--[표현식]
/*
 * 
 * CRATE TABLE 테이블명( 컬럼명 자료형(크기), ....);
 * 
 * 
 * 
 * 
 ...
 * --> EX) VARCHAR2(10) 컬럼에 'ABC' 3BYE 문자열만 저장하면 나머지 7BYTE 반환함
 * 
 *  DATE : 날짜 타입
 *  BLOB : 대용량 이진 데이터(4GB) 
 * 	CLOB : 대용량 문자 데이터(4GB)
...
-- SQL작성법 : 대문자 작성권장, 연결된 다언 사이는 언더바 사용 
...
-- INSERT TO 테이블명 VALUSE(값1, 값2.....);

INSERT INTO MEMBER VALUES('MEMO1', '123A', '홍길동', '991213-1234567',DEFAULT);

SELECT *FROM "MEMBER";

-- * INSERT/UPDATE 시 컬럼값으로 DEFAULT를 작성하면 
-- 테이블 생성시 해당 컬럼에 지정된 DEFAULT 값으로 삽입이 된다
INSERT INTO MEMBER VALUES('MEMO2', '123AA', '김길동', '901213-1234567',DEFAULT);
INSERT INTO MEMBER VALUES('MEMO3', '123DA', '조길동', '961213-1234567',SYSDATE);


INSERT INTO MEMBER(MEMBER_ID, MEMBER_PWD,MEMBER_NAME)
VALUES('MEMO4', '223DA', '박길동');


-- JDBC 에서 날짜를 입력받았을때 삽입하는 방법
-- '2022-08-14 17:33:33'
INSERT INTO MEMBER VALUES('MEMO5', '143A', '마길동', '201213-1234567',
TO_DATE('2022-08-14 17:33:33', 'YYYY-MM-DD HH24:MI:SS'));
...
INSERT INTO MEMBER2 VALUES('MEMO1', '123AA', '김길동', 5555555);
INSERT INTO MEMBER2 VALUES('MEMO2', '123AA', '동길동', 0555555);
 ...
 * --> 중복 데이터X
 * 
 * + 입력 데이터에 문제가 없는 자동으로 검사하는 목적
 * 데이터의 수정/삭제 가능 여부 검사등을 목적으로 함 
 * -> 제약 조건을 위배하는 DML 구문은 수행할 수 없다
 * 
 * 제약조건 종류
 * PRIMARY KEY, NOT NULL, UNIQE, CHECK, FOREIGN KEY
 * 
 * 
 * 
 */
...
-- 삽입/ 수정시 NULL 값을 허용하지 않도록 컬럼레벵에서 제한 
...
VALUES(1,'USERO1', 'PAS01', '홍길동','남','010-4873-1234','hon111@naer.vo');


INSERT INTO USER_USED_MN
VALUES(NULL,'USERO1', 'PAS01', '김길동','남','010-4873-1234','hon111@naer.vo');
--SQL Error [1400] [23000]: ORA-01400: NULL을 ("KH"."USER_USED_MN"."USER_NO") 안에 삽입할 수 없습니다
...
-- 단, UNIQE 제약 조건이 설정된 컬럼에 NULL 값은 중복 삽입 가능 

-- 테이블 레벨 : 테이블 생성 시 컬럼 정의가 끝난후 마지막 작성
...
CREATE TABLE USER_USED_UK(
   ...
   -- USER_ID VARCHAR2(20) UNIQUE, -- 컬럼레벨(제약조건명 미지정)
	--USER_ID VARCHAR2(20) CONSTRAINT USER_ID_U UNIQUE -- 컬럼레벨 (제약 조건명 지정 )
	 USER_ID VARCHAR2(20),
	USER_PWD VARCHAR2(20),
	USER_NAME VARCHAR2(30),
	GENDER VARCHAR2(10),
...
-- 같은 아이디인 데이터가 이미 테이블에 있으면 UNQUE 제약 조건에 위배되어 오류 발생

INSERT INTO USER_USED_MN
VALUES(1,NULL, 'PAS01', '홍길동','남','010-4873-1234','hon111@naer.vo');
...
INSERT INTO USER_USED_UK2
VALUES(1,'USERO2', 'PAS01', '홍길동','남','010-4873-1234','hon111@naer.vo');
...
-- NOT NULL + UNIQUE 제약 조건의 의미 -- > 중복되자 않는 값이 필수로 존재해야한다 

-- 한 테이블당 한개만 설정할 수 있다 
-- 컬럼레벨, 테이블 레벨 둘다 설정 가능하다 
-- 한개 컬럼에 설정할 수 도 있고 여러개의 컬럼을 묶어서 설정 할 수 있다 
...
INSERT INTO USER_USED_PK
VALUES(1,'USERO2', 'AAS01', '단길동','남','010-4573-1234','h4n111@naer.vo');
--무결성 제약 조건(KH.USER_NO_PK)에 위배됩니다
--> 기본 키 중복으로 오류


INSERT INTO USER_USED_PK
VALUES(NULL,'USERO2', 'AAS01', '단길동','남','010-4573-1234','h4n111@naer.vo');
...
CREATE TABLE USER_USED_PK2(
	...
	PHONE VARCHAR2(30),
	EMAIL VARCHAR2(50),
	CONSTRAINT USER_NO_ID_PK PRIMARY KEY (USER_ID, USER_NAME) -- 테이블 레벨(제약조건명 지정)
);
...
-- FOREIGN KEY 제약조건에 의해서 테이블간 관계가 형성된다 
-- 제공하는 값 외에는 NULL(참조하는 값 없다 )을 사용할 수 있다 
...
-- * 참조될 수 있는 컬럼은 PRIMART KEY 컬럼과, UNIQUE 지정된 컬럼만 외래키로 사용할 수 있다 
-- 참조할 테이블의 참조할 컬럼명이 생략되면, PRIMARY KEY로 설정된 컬럼이 자동 참조할 컬럼이 된다 
...
CREATE TABLE USER_USED_FK(
		...
		--									컬럼명 미작성시 USER_GRADE 테이블의 PK 자동참조
		--테이블레벨
...
VALUES(1,'USERO1', 'PAS01', '홍길동','남',10);



INSERT INTO USER_USED_FK
VALUES(2,'USERO2', 'PAS01', '홍길동','남',10);


INSERT INTO USER_USED_FK
VALUES(3,'USERO3', 'PAS01', '홍길동','남',30);


INSERT INTO USER_USED_FK
VALUES(4,'USERO4', 'PAS01', '홍길동','남',NULL);


INSERT INTO USER_USED_FK
VALUES(5,'USERO5', 'PAS01', '홍길동','남',50);
-- ORA-02291: 무결성 제약조건(KH.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
-- > 50이라는 값은 USER_GRAD 테이블 GRADE_CODE 칼럼에서 제공하는 값이 아니므로 
...
-- 어떤식으로 처리할지 에 대한 내용를 설정할 수 있다 

--1) ON DELETE RESTRICTED(삭제 제한)로 기본 지정 되어있다 
...
-- GRADE_CODE 중 20은 외래키로 참조되고 있지 않았으므로 삭제가 가능하다 
...
-- 부모 키 삭제시 값을 사용하는 자식 테이블의 컬럼에 해당하는 행이 삭제가 됨
...
--- 5. CHECK 제약 조건 : 컬럼에 기록되는 값에 조건 설정을 할 수 있다 
-- CHECK (컬럼명 비교연산자 비교값)
-- 주의 : 비교값은 리터럴만 사용할 수 있다 변하는 값이나 함수는 사용 못함

CREATE TABLE USER_USED_CKEK(
 		USER_NO NUMBER PRIMARY KEY,
 		USER_ID VARCHAR2(20) UNIQUE, 
		USER_PWD VARCHAR2(30) NOT NULL,
		USER_NAME VARCHAR2(30),
...
INSERT INTO USER_USED_CKEK
VALUES(1,'USERO1', 'PAS01', '홍길동','남',10);


INSERT INTO USER_USED_CKEK
VALUES(1,'USERO1', 'PAS01', '홍길동','남자',10);
-- GENDER 칼럼에 CHECK 제약 조건으로 '남' 또는 '여'만 기록 가능한데 '남자'라는 조건외 값이 들어와 에러 발생
...
COMMENT ON COLUMN USER_TESTT IS '회원번호';



INSERT INTO USER_TESTT
VALUES(1, 'USER1', 'AAAA', '970000-1093454','남','010-3567-9134','서울시 강북구','N');
INSERT INTO USER_TESTT
VALUES(2, 'USER2', 'AAAB', '870000-1093454','남','010-3567-9534','김포시 강북구','Y');
INSERT INTO USER_TESTT
VALUES(3, 'USER3', 'AAAA', '070000-1093454','남','010-3567-9134','안산시 동창구','N');
INSERT INTO USER_TESTT
VALUES(4, 'USER4', 'AAAA', '170000-1093454','남','010-3567-9134','서울시 성북구','N');
INSERT INTO USER_TESTT
VALUES(5, 'USER5', 'AAAA', '770000-1093454','남','010-3567-9134','경북시 강북구','N');


SELECT*FROM USER_TESTT;
...
--> 서브 쿼리의 조회 결과(RESULT SET)의 모양대로 테이블이 생성됨 
...
--> 제약조건, 코멘트는 복사되지 않기때문에 별도 추가 작업이 필요하다 
...
---DEPARTMENT 테이블의 LOCATTION_ID 외래키 제약조건 추가
-- 참조 테이블은 LOCATION, 참조컬럼은 LOCATION의 기본키 
ALTER TABLE DEPARTMENT ADD CONSTRAINT LOCATTION_ID
FOREIGN KEY(LOCATTION_ID) REFERENCES LOACTION ON DELETE SET NULL;
