...
-- 그룹 함수 :  N개의 값을 읽어서 1개의 갤과를 반환(합계, 평균, 최대, 최소)

-- 함수는 SELECT 문의 
-- SELECT, WHERE절, ORDER BY절, HAVING 절 사용 가능 
...
-- MOD(숫자 | 컬럼명, 숫자|컬럼명) : 나머지 값 반환
 ...
 -- EMPLOYEE 테이블에서 사원의 월큽을 100만으로 나눴을때 나머지 조회
...
-- TRUNC(숫자 | 컬럼명 [,위치]) : 특정위치 아래를 버림(첨삭)

SELECT TRUNC(123.456) FROM DUAL; -- 123, 소수점 아래를 버림

SELECT TRUNC(123.456,1) FROM DUAL; -- 123.4 소수점 첫재 아래를 버림
...
SELECT ROUND (MONTHS_BETWEEN(SYSDATE,'2023-12-22' ),3) "수강 기간(개월)"
FROM DUAL; -- 4.568 -- ABS 앞에 붙이면 절대값으로 양수가 나옴
...
SELECT LAST_DAY('2020-03-01')FROM DUAL;
...
SELECT TO_CHAR(1000000, 'l9,999,999') FROM DUAL; -- '원1,000,000'
...
-- 년,월,일이 날짜를 나타내느 패턴으로 인식이 안되서 오류 발생
-- "" 쌍옴표를 이용해서 단순한 문자로 인식시키면 해결됨
...
-- TO_DATE(문자형 데이터, [포맷]) : 문자형 데이터를 날싸로 변경
...
--'YYMMDD HH24MISS' 패턴을 적용해서 작성된 문자열의 각 문자가 어떤 날짜 형식인지 인지시킴
...
-- TO_NUMBER(문자 데이터, [포멧]) : 문자형데이터 숫자 데이터로 변경
...
/* NULL 처리 함수
 ...
 -- NVL (컬럼명, 컬럼값이 NULL일때 바꿀 값) : NULL인 컬럼 값을 다른 값으로 변경
...
-- DECODE(계산식 또는 컬럼명, 조건값 1, 선택값1, 조건값2, 선택값2...., 아무것도 일치하지 않을때)
-- 비교하고자 하는 값 또는 컬럼이 조건식이 같으면 결과 값 반환 
...
DECODE(JOB_CODE, 'J7', '20%','5%') 인상률,
DECODE(JOB_CODE,'J7', SALARY*1.2,SALARY *1.05)"인상된 급여" 
FROM EMPLOYEE;
...
---------------
/*  그룹 함수
 * -- 하나 이상의 행을 그룹으로 묶어 연산하여 총합, 평균 등의 하나의 결괄 행으로 반환하는 함수 
 */
...
-- 급여 최대값, 가장 늦은 입사일, 알바펫 순서가 가장 느린 이메일
