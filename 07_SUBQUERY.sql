...
ORDER BY JOB_CODE;
--SELECT 절에 없는 컬럼이라도 FROM, JOIN으로 인해 존재하는 컬럼이면 ORER BY절에 사용가능 
...
--***
SELECT EMP_ID,EMP_NAME,NVL((SELECT EMP_NAME 
FROM EMPLOYEE SUB WHERE SUB.EMP_ID = MAIN.MANAGER_ID),'없음') 관리자명 
FROM EMPLOYEE MAIN
