...
-- 컬럼 수를 기준으로 JOIN
--> 왼편에 작성된 테이블의 모든 행이 결과에 포함되어야 한다(JOIN이 안되는 행도 결과포함)
...
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
CROSS JOIN DEPARTMENT; -- 207행 = 23행 * 9행
...
--> 다중조인시 앞에서 조인된 결과에 새로운 테이블 내용을 조인 

SELECT *FROM  EMPLOYEE
JOIN LOCATION ON(LOCATION = LOCAL_CODE)
JOIN DEPARTMENT ON(DEPT_CODE = DETP_ID);
-- EMPLOYEE 테이블에 LACATTION_ID 컬럼이 없어서 오류발생
--> 해결 방법 : DEPARTMENT 와 LOCATION 조인 순서를 바꿔서 
--				EMPLOYEE와 DEPAMENT가 조인된 결과를 먼저 만들어
-- 				LOCATION_ID 칼럽이 존재할수 있도록 만든다
...
JOIN JOB USING (JOB_CODE)
WHERE EMP_NO LIKE '7%'
AND EMP_NO LIKE '_______2%' --SUBSTR
AND EMP_NAME LIKE '전%'
...
FROM EMPLOYEE  
JOIN JOB USING (JOB_CODE) --"NATIONAL"
JOIN DEPARTMENT ON ( DEPT_ID =DEPT_CODE)
WHERE EMP_NAME LIKE '%형%'  -----%A% 이름 찾기
...
JOIN JOB USING (JOB_CODE)
JOIN DEPARTMENT ON ( DEPT_ID =DEPT_CODE)
WHERE LOCATION_ID IN ('L2','L3') ---DEPT_TITLE IN('해외영업1부')
-- OR절 -- DEPT_TITLE LIKE '%1부' OR LIKE '%2부'
...
LEFT JOIN LOCATION  ON (LOCAL_CODE= LOCATION_ID) --- 부서 없는 하동훈 누락되서 LEFT 써야함
WHERE BONUS IS NOT NULL

--5. 부서가 있는 사원의 사원명, 직급명, 부서명, 지역명 조회

SELECT EMP_NAME, JOB_NAME,DEPT_CODE,DEPT_TITLE, LOCAL_NAME
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
JOIN DEPARTMENT ON ( DEPT_ID =DEPT_CODE)
...
FROM EMPLOYEE E1
JOIN EMPLOYEE E2 ON(E2.DEPT_CODE=E1.DEPT_CODE)
WHERE E1.EMP_NAME != E2.EMP_NAME --- 자기이름 빼고
ORDER BY 사원명;
