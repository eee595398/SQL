...
FROM EMPLOYEE 
GROUP BY DEPT_CODE; -- DEPT CODE가 같은 행끼리 하나의 그룹이 됨 
;
...
ORDER BY "인원 수"; -- 별칭 사용 0(SELECT절 해석 완료)
...
--EMPLOYEE 테이블에서 부서코드가 D5, D6인 부서별의 평균 급여, 인원수 조회
...
-- HAVING 컬럼 명 | 함수식 비교연산자 비교값

-- 부서 별 평균 급여가 3백만원 이상인 부서를 조회

SELECT DEPT_CODE, AVG(SALARY)
FROM EMPLOYEE
--WHERE SALARY >= 3000000 -> 한사람의 급여가 300만 이상이라는 조건
GROUP BY DEPT_CODE
...
UNION
SELECT DEPT_ID, DET_TITLE FROM DEPARTMENT
